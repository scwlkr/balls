...
usage() {
  echo 'Usage: eng/conformance/Test-WindowsSmbReadiness.sh --target-profile <json> --receipt <json>' >&2
  exit 2
}
...
for required_command in dotnet git scp ssh timeout; do
  if ! command -v "$required_command" >/dev/null 2>&1; then
    echo "windows-conformance: required_command_missing_$required_command" >&2
    exit 3
  fi
done
...
work_directory=$(mktemp -d -t balls-windows-conformance.XXXXXXXXXX)
cleanup() {
...
timeout --foreground --signal=INT --kill-after=75s 8m \
