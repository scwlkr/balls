...
public sealed class MacOSUnixSocketControl :
    Balls.Platform.ILocalControlServerTransport,
    Balls.Platform.ILocalControlClientTransport
{
    private const int MaximumSocketPathBytes = 103;
    ...
    public void ValidateEndpoint(string endpoint)
    {
            ...
            || !string.Equals(endpoint, Path.GetFullPath(endpoint), StringComparison.Ordinal))
        {
            throw new ArgumentException(
                "The macOS local-control socket must use a normalized absolute path.",
                nameof(endpoint));
        }
        ...
        {
            throw new ArgumentException(
                $"The macOS local-control socket path cannot exceed {MaximumSocketPathBytes} UTF-8 bytes.",
                nameof(endpoint));
        }
    }
    ...
    public void PrepareEndpoint(string endpoint)
    {
        ...
        MacOSDataDirectorySecurity.EnsurePrivateRuntimeDirectory(parent);

        var existing = MacOSNativeFileSystem.TryReadStatus(endpoint);
        if (existing is null)
        {
            return;
        }

        if (!existing.IsSocket || existing.UserId != MacOSNativeFileSystem.EffectiveUserId)
        {
        ...
        try
        {
            ...
            exception.SocketErrorCode is SocketError.ConnectionRefused or SocketError.AddressNotAvailable)
        {
    ...
    {
        var status = MacOSNativeFileSystem.ReadStatus(endpoint);
        if (!status.IsSocket || status.UserId != MacOSNativeFileSystem.EffectiveUserId)
        {
            throw new UnauthorizedAccessException(
                "The bound local-control socket is not owned by the current macOS user.");
        }
    ...
    {
        ValidateEndpoint(endpoint);
        var status = MacOSNativeFileSystem.TryReadStatus(endpoint);
        if (status is null)
        {
            return;
        }
