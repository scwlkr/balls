...
internal static class BrowserAdapter
{
    ...
    public static void MapRoutes(
        ...
        CircleMessageApplication messageApplication,
        BrowserAccessBroker access)
    {
        application.MapPost(
        ...
        application.MapGet(
                ...
                async (string circleId, CancellationToken token) =>
                {
                    ...
                    var messages = await messageApplication.ListAsync(
                        circle.Circle.Id,
                        token).ConfigureAwait(false);
                    return Results.Ok(
                        new CircleMessageListResponse(
                            circle.Circle.Id.ToString(),
                            messages.Select(message => new CircleMessageResponse(
                                message.Id.ToString(),
