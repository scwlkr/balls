...
| Windows inspection hangs, floods output, or returns malformed/forward-unknown data | The child process has a 10-second timeout, 64 KiB output cap, strict JSON parsing, exact recognized values, and deterministic redacted `unknown` results | Inspection can be unavailable or conservatively unknown; this endpoint is not a general diagnostic console |
| SMB would be exposed on an unsafe network scope | Readiness is `not-ready` unless a connected Private profile exists and both Private/Public firewall profiles are enabled with default inbound Block | Readiness proves enforceable preconditions, not that a later helper creates a correct narrowly scoped rule |
