...
| Circle Files metadata leaks provider secrets or private authority | Core records contain no provider credentials; local-control and CLI projections omit transcripts, signatures, passwords, and private material; the browser Member-open body is empty and its response omits endpoint, drive, grant, plan, provider, account, and credential data while retaining antiforgery state in memory | Detailed endpoint, drive, lifecycle, Member/Node relationships, and authorization times remain intentionally inspectable to the same OS account through protected local-control diagnostics |
...
| A hostile or substituted folder path reaches privileged mutation | The normal Windows picker runs unelevated; `ballsd` binds its exact result and an opaque selection ID to the authenticated browser session and Circle for 15 minutes, while apply accepts no browser path/name and rejects missing, stale, replaced, cross-session, or request-ID-substituted state. Both privilege levels independently canonicalize the selected absolute fixed-local path and reject drive roots, Windows/profile roots, network locations, files, existing reparse traversal, and foreign or reserved Balls metadata; the elevated helper reruns preflight immediately before mutation | Same-user and administrator races remain possible without retained directory handles; the helper fails closed on substituted paths or ownership witnesses |
...
| Partial failure leaves unsafe access or rollback deletes user state | An operation journal records whether the exact folder pre-existed plus its prior ACL before restriction; it never claims ordinary child entries. All resource collisions are preflighted; failure and partial-state recovery inspect exact ownership and roll back in reverse order; only proven-owned rules, shares, markers, and journal files are removed, the observed prior ACL is restored, and a pre-existing folder plus every ordinary file remains in place | Power loss or hostile interference can require a later retry; a recovery-incomplete result fails closed instead of broad cleanup |
...
- Circle Files Contributions currently remain `defined`. A Windows Owner can select an existing
  fixed-local folder and create the exact owned share/firewall resources, one limited account plus
  exact ACL per Access Grant, and an
  ...
  delete or replicate existing content, or prove remote/physical-machine access. The local Owner remains the host
