...
| Cleanup removes foreign or user-owned state | Every rollback step re-inspects recorded account/ownership IDs, protected markers, ACL entries, share/rule descriptions, and exact properties; ambiguous, blocked, or substituted state refuses mutation. The known Windows share-creation firewall side effect is restored to its proven disabled baseline before exact share removal, including retry after interruption. Host cleanup never calls folder/content deletion, and Windows integration preserves exact user-file bytes | An administrator can forge all local evidence; secure deletion and recovery/import UX are out of scope |
