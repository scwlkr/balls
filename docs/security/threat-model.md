...
| A network endpoint or wrong share is treated as Circle authority | Mapping accepts only canonical numeric private/loopback IPv4, derives the share/account from signed local state, and reads both exact host and grant ownership markers through SMB before success; endpoint and SMB metadata grant no authority | A host administrator can forge local marker files and Windows resources; authenticated remote replication remains separate work |
| Mapping overwrites or later removes another resource | The user explicitly selects D-Z; occupied drive, credential target, or Explorer label collisions fail before mutation. Retry and unmap compare exact UNC, account, ownership comment/marker, and friendly name; unmap is non-forced and preserves changed/open mappings | Windows does not return the stored domain-password blob on every supported configuration; ownership therefore relies on the random derived comment plus target/account, while same-user races and administrator manipulation remain possible |
| Provider password leaks while mapping | The local API/browser/CLI never receive the password. The daemon unprotects it only into disposable buffers, writes it to current-user Credential Manager, and passes it directly to Win32 in process rather than process arguments or shell commands | The Windows networking API consumes plaintext in daemon memory; same-user compromise, administrator/LocalSystem, debugger, crash dump, or Credential Manager compromise remain powerful |
