...
| Elevated subprocess hangs, floods output, or leaks diagnostics | Helper IPC is capped at 64 KiB; the helper has an independent two-minute lifetime; each fixed PowerShell child has a 20-second timeout and one combined 16,384-character streaming budget, suppresses inherited module paths, and returns only typed states and bounded public errors | Local Windows tooling can still be unavailable or compromised; raw subprocess output is intentionally discarded |
