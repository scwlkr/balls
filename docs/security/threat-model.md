...
| Caller input becomes an arbitrary elevated command | The daemon launches one exact adjacent helper and passes only a random pipe name and daemon PID; the helper accepts one versioned hosting plan, recomputes it under elevation, and selects only fixed typed folder/share/firewall operations and a fixed encoded PowerShell command enum | Replacement of the daemon/helper binary by the same user or an administrator remains outside this source-run boundary; installed artifacts require protected locations and signing |
| Another process impersonates the daemon or elevated helper | A cryptographically random pipe name is never persisted; before any plan is sent, the daemon verifies the connected client PID equals the exact launched helper PID and the helper verifies the pipe server PID equals the passed daemon PID; messages are length-prefixed and capped at 64 KiB | Administrator/LocalSystem and compromise of either authenticated process remain powerful |
...
| Partial failure leaves unsafe access or rollback deletes user state | An operation journal records folder provenance before restriction; all resource collisions are preflighted; failure and partial-state recovery inspect exact ownership and roll back in reverse order; only proven-owned rules/shares/markers are removed, newly created directories are removed only while empty, and an originally empty folder's ACL is restored | Power loss or hostile interference can require a later retry; a recovery-incomplete result fails closed instead of broad cleanup |
...
| Elevated subprocess hangs, floods output, or leaks diagnostics | Helper IPC is capped at 64 KiB; the fixed PowerShell child has a 20-second timeout and one combined 16,384-character streaming budget, suppresses inherited module paths, and returns only typed states and bounded public errors | Local Windows tooling can still be unavailable or compromised; raw subprocess output is intentionally discarded |
