...
| Mapping overwrites or later removes another resource | The user explicitly selects D-Z; occupied drive, credential target, or Explorer label collisions fail before mutation. Retry and unmap compare exact UNC, account, ownership comment/marker, friendly name, and fixed-time secret bytes; unmap is non-forced and preserves changed/open mappings | Same-user races and administrator manipulation remain possible; exact retry is required after interruption |
